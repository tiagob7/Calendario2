(function() {
  ...
  window.bootProtectedPage({
    ...
    if (isAdmin) {
      document.getElementById('filterEscritorio').style.display = '';
    }
  ...
  function renderList() {
    ...
    if (!lista.length) {
      el.innerHTML = '<div class="empty-state">Nenhuma visita encontrada.</div>';
      return;
    }
  ...
  function cardHtml(v) {
    ...
    return `
      <div class="visita-row estado-${window.escHtml(v.estado)}" id="card_${v.id}">
        <div class="visita-row-head" onclick="toggleCard('${v.id}')">
          <div>
            <div class="row-nome">${window.escHtml(v.cliente || '—')}</div>
            <div class="row-sub">${window.escHtml(data)} · ${window.escHtml(v.local || '—')} · ${window.escHtml(v.nomeCompleto || v.email || '—')}</div>
          </div>
        ...
        <div class="visita-row-body" id="body_${v.id}">
          ...
          ${v.descricao ? `<div class="row-desc">${window.escHtml(v.descricao)}</div>` : ''}
          ${v.resultado ? `<div class="row-resultado"><div class="row-resultado-label">Resultado</div>${window.escHtml(v.resultado)}</div>` : ''}
          ${acoes.length ? `<div class="row-actions">${acoes.join('')}</div>` : ''}
        </div>
      </div>`;
  }
  ...
  window.openForm = function() {
    document.getElementById('visitaFormModal').classList.add('open');
  };
  window.closeForm = function() {
    document.getElementById('visitaFormModal').classList.remove('open');
  };

  window.setFiltro = function(f) {
    filtroEstado = f;
    document.querySelectorAll('.seg-btn[data-f]').forEach(b => b.classList.toggle('active', b.dataset.f === f));
    renderList();
  };
  ...
  document.addEventListener('keydown', e => { if(e.key === 'Escape') window.closeForm && window.closeForm(); });
})();
