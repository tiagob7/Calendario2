(function() {
  ...
  function cardHtml(d) {
    ...
    return `
      <div class="despesa-row estado-${window.escHtml(d.estado)}" id="card_${d.id}">
        <div class="despesa-row-head" onclick="toggleCard('${d.id}')">
          <div>
            <div class="row-nome">${window.escHtml(d.descricao || '—')}</div>
            <div class="row-sub">${window.escHtml(d.nomeCompleto || d.email || '—')} · ${window.escHtml(data)}</div>
          </div>
          <span class="cat-tag">${window.escHtml(catLabel)}</span>
          <span class="row-valor">${window.escHtml(valor)} €</span>
          <span class="estado-pill ${window.escHtml(d.estado)}">${window.escHtml(d.estado)}</span>
        </div>
        <div class="despesa-row-body" id="body_${d.id}">
          ...
          ${reciboHtml}
          ${d.observacao ? `<div class="row-obs">Obs: ${window.escHtml(d.observacao)}</div>` : ''}
  ...
  window.openForm = function() {
    document.getElementById('despesaFormModal').classList.add('open');
  };
  window.closeForm = function() {
    document.getElementById('despesaFormModal').classList.remove('open');
  };
